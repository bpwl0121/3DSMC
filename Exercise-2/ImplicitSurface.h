...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d vec2Center=_x-m_center; 
		return vec2Center.squaredNorm()-pow(m_radius,2);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d vec2Center=_x-m_center;
		double m_radius_square=pow(m_radius,2);
		double m_a_square=pow(m_a,2);
		double result=pow(vec2Center.squaredNorm()+m_radius_square-m_a_square,2)-4*m_radius_square*(pow(vec2Center[0],2)+pow(vec2Center[1],2));
		return result;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for(int idx=0;idx<m_numCenters;idx++){
			result+=EvalBasis((m_funcSamp.m_pos[idx]-_x).norm())*m_coefficents[idx];
		}
		result+=_x[0]*m_coefficents[m_numCenters]+_x[1]*m_coefficents[m_numCenters + 1]+_x[2]*m_coefficents[m_numCenters + 2];
	...
	void BuildSystem()
	{
		...
		for(int i=0; i<2*m_numCenters; i++){
			for(int j=0; j<(m_numCenters+4); j++){
				...
				else if(j==m_numCenters) A(i,j)=m_funcSamp.m_pos[i][0];
				else if(j==m_numCenters+1) A(i,j)=m_funcSamp.m_pos[i][1];
				else if(j==m_numCenters+2) A(i,j)=m_funcSamp.m_pos[i][2];
