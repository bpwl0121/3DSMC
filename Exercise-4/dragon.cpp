...
struct RegistrationCostFunction
{
	...
	{
		T residual1=(cos(theta[0])*point1.x)-(sin(theta[0])*point1.y)+tx[0]-point2.x;
		T residual2=(sin(theta[0])*point1.x)+(cos(theta[0])*point1.y)+ty[0]-point2.y;
		residual[0]=weight.w*(residual1*residual1+residual2*residual2);
		return true;
	}
