...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // FIXME PoseIncrement<T> poseIncrement(pose);
        ...
        residuals[0] = T(LAMBDA)*T(m_weight)*(targetPoint[0]-T(sourcePoint[0]));
		residuals[1] = T(LAMBDA)*T(m_weight)*(targetPoint[1]-T(sourcePoint[1]));
		residuals[2] = T(LAMBDA)*T(m_weight)*(targetPoint[2]-T(sourcePoint[2]));
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        T x = T(m_targetNormal[0]) * (targetPoint[0] - T(m_targetPoint[0]));
		T y = T(m_targetNormal[1]) * (targetPoint[1] - T(m_targetPoint[1]));
		T z = T(m_targetNormal[2]) * (targetPoint[2] - T(m_targetPoint[2]));
        // residuals[0] = T(LAMBDA)*T(m_weight)*(T(m_targetPoint)-targetPoint).dot(T(m_targetNormal));

        residuals[0] = T(LAMBDA)*T(m_weight)*(x+y+z);
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                float angle=acos(sourceNormal.dot(targetNormal));
                if(angle>M_PI/3) match.idx=-1;
            }
        }
    }
};
    ...
    void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
        ...
        for (unsigned i = 0; i < nPoints; ++i) {
            ...
            if (match.idx >= 0) {
                ...
                problem.AddResidualBlock(PointToPointConstraint::create(sourcePoint, targetPoint, weight),nullptr,poseIncrement.getData());
                ...
                if (m_bUsePointToPlaneConstraints) {
                    ...
                    // to the Ceres problem.
                    problem.AddResidualBlock(PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal,weight),nullptr,poseIncrement.getData());
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            /* 
                ...
                -Sz   0  Sx 0 1 0
                 Sy -Sx   0 0 0 1  
            */
            Matrix3f point2PointA;
			point2PointA<<0,s[2],-s[1],-s[2],0,s[0],s[1],-s[0],0;
            A.block(4*i+1, 0, 3, 3) = point2PointA;
            A.block(4*i+1, 3, 3, 3) = Matrix3f::Identity();
            b.block(4*i+1, 0, 3, 1) = d-s;
        ...
        VectorXf x(6);
        JacobiSVD<MatrixXf> svdSolver(A, ComputeFullU | ComputeFullV);
		x=svdSolver.solve(b);
